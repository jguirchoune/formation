...
class AnnonceAccessControlHandler extends EntityAccessControlHandler {
  ...
  protected function checkAccess(EntityInterface $entity, $operation, AccountInterface $account) {
    ...
    switch ($operation) {
      ...
      case 'update':
          if($account->id() == $entity->getOwnerId()) return AccessResult::allowedIfHasPermission($account, 'edit annonce entities');
          else return AccessResult::forbidden();

      case 'delete':
          if($account->id() == $entity->getOwnerId()) return AccessResult::allowedIfHasPermission($account, 'delete annonce entities');
          else return AccessResult::forbidden();
    }
